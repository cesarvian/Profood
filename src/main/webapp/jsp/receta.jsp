...
<%    
    ...
    if (request.getParameter("listar").equals("cargar")) {
        ...
        //cantidad
        //precio
        String codproveedor = request.getParameter("codproducto");
        ...
        try {
            ...
            rs = st.executeQuery("SELECT id FROM recetas where estado='Pendiente';");
            if (rs.next()) {
                //out.println("Existe cabecera");
                st.executeUpdate("insert into detallerecetas (idreceta,idingrediente,cantidad)values('" + rs.getString(1) + "','" + codproducto + "','" + cantidad + "')");
            } else {
                //out.println("NO existe cabecera");

                st.executeUpdate("insert into recetas (idproducto,fecha,iduser,usuario)values('" + codproducto + "','" + fecharegistro + "','"+sesion.getAttribute("id")+"','"+sesion.getAttribute("usser")+"')");
                pk = st.executeQuery("SELECT id FROM recetas where estado='Pendiente';");
                pk.next();
                st.executeUpdate("insert into detallerecetas (idcompra,idproducto,cantidad)values('" + pk.getString(1) + "','" + codproducto + "','" + cantidad + "')");
